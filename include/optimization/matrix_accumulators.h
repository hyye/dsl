...
#ifndef DSL_MATRIX_ACCUMULATORS_H_
#define DSL_MATRIX_ACCUMULATORS_H_
...
class AccumulatorXX {
 public:
  ...
  inline void Initialize() {
    ...
    A1k.setZero();
    A1m.setZero();
    num = num_in_1 = num_in_1k = num_in_1m = 0;
  }

  inline void Finish() {
    ShiftUp(true);
    num = num_in_1 + num_in_1k + num_in_1m;
  }
 ...
 private:
  float num_in_1, num_in_1k, num_in_1m;

  void ShiftUp(bool force) {
    if (num_in_1 > 1000 || force) {
      A1k += A;
      A.setZero();
      num_in_1k += num_in_1;
      num_in_1 = 0;
    }
    if (num_in_1k > 1000 || force) {
      A1m += A1k;
      A1k.setZero();
      num_in_1m += num_in_1k;
      num_in_1k = 0;
...
class Accumulator11 {
 public:
  ...
  inline void Initialize() {
    A = 0;
    memset(SSE_data, 0, sizeof(float) * 4 * 1);
    memset(SSE_data1k, 0, sizeof(float) * 4 * 1);
    memset(SSE_data1m, 0, sizeof(float) * 4 * 1);
    num = num_in_1 = num_in_1k = num_in_1m = 0;
  }

  inline void Finish() {
    ShiftUp(true);
    A = SSE_data1m[0 + 0] + SSE_data1m[0 + 1] + SSE_data1m[0 + 2] +
        SSE_data1m[0 + 3];
  }
  ...
  inline void UpdateSSE(const __m128 val) {
    _mm_store_ps(SSE_data, _mm_add_ps(_mm_load_ps(SSE_data), val));
    num += 4;
    num_in_1++;
    ShiftUp(false);
  }
 ...
 private:
  EIGEN_ALIGN16 float SSE_data[4 * 1];
  EIGEN_ALIGN16 float SSE_data1k[4 * 1];
  EIGEN_ALIGN16 float SSE_data1m[4 * 1];
  float num_in_1, num_in_1k, num_in_1m;

  void ShiftUp(bool force) {
    if (num_in_1 > 1000 || force) {
      _mm_store_ps(SSE_data1k,
                   _mm_add_ps(_mm_load_ps(SSE_data), _mm_load_ps(SSE_data1k)));
      num_in_1k += num_in_1;
      num_in_1 = 0;
      memset(SSE_data, 0, sizeof(float) * 4 * 1);
    }
    ...
    if (num_in_1k > 1000 || force) {
      _mm_store_ps(SSE_data1m, _mm_add_ps(_mm_load_ps(SSE_data1k),
                                          _mm_load_ps(SSE_data1m)));
      num_in_1m += num_in_1k;
      num_in_1k = 0;
      memset(SSE_data1k, 0, sizeof(float) * 4 * 1);
    }
...
class Accumulator14 {
 public:
  ...
  inline void Initialize() {
    H.setZero();
    b.setZero();
    memset(SSE_data, 0, sizeof(float) * 4 * 105);
    memset(SSE_data1k, 0, sizeof(float) * 4 * 105);
    memset(SSE_data1m, 0, sizeof(float) * 4 * 105);
    num = num_in_1 = num_in_1k = num_in_1m = 0;
  }
  ...
  inline void Finish() {
    ...
    for (int r = 0; r < 14; r++)
      for (int c = r; c < 14; c++) {
        float d = SSE_data1m[idx + 0] + SSE_data1m[idx + 1] +
                  SSE_data1m[idx + 2] + SSE_data1m[idx + 3];
        H(r, c) = H(c, r) = d;
        idx += 4;
      }
    assert(idx == 4 * 105);
 ...
 private:
  EIGEN_ALIGN16 float SSE_data[4 * 105];
  EIGEN_ALIGN16 float SSE_data1k[4 * 105];
  EIGEN_ALIGN16 float SSE_data1m[4 * 105];
  float num_in_1, num_in_1k, num_in_1m;

  void ShiftUp(bool force) {
    if (num_in_1 > 1000 || force) {
      for (int i = 0; i < 105; i++)
        _mm_store_ps(SSE_data1k + 4 * i,
                     _mm_add_ps(_mm_load_ps(SSE_data + 4 * i),
                                _mm_load_ps(SSE_data1k + 4 * i)));
      num_in_1k += num_in_1;
      num_in_1 = 0;
      memset(SSE_data, 0, sizeof(float) * 4 * 105);
    }
    ...
    if (num_in_1k > 1000 || force) {
      for (int i = 0; i < 105; i++)
        _mm_store_ps(SSE_data1m + 4 * i,
                     _mm_add_ps(_mm_load_ps(SSE_data1k + 4 * i),
                                _mm_load_ps(SSE_data1m + 4 * i)));
      num_in_1m += num_in_1k;
      num_in_1k = 0;
      memset(SSE_data1k, 0, sizeof(float) * 4 * 105);
    }
...
class Accumulator9 {
 public:
  ...
  inline void Initialize() {
    H.setZero();
    b.setZero();
    memset(SSE_data, 0, sizeof(float) * 4 * 45);
    memset(SSE_data1k, 0, sizeof(float) * 4 * 45);
    memset(SSE_data1m, 0, sizeof(float) * 4 * 45);
    num = num_in_1 = num_in_1k = num_in_1m = 0;
  }
  ...
  inline void UpdateSSEEighted(const __m128 J0, const __m128 J1,
 ...
 private:
  EIGEN_ALIGN16 float SSE_data[4 * 45];
  EIGEN_ALIGN16 float SSE_data1k[4 * 45];
  EIGEN_ALIGN16 float SSE_data1m[4 * 45];
  float num_in_1, num_in_1k, num_in_1m;

  /// shift SSE_data to 1k or 1m level
  /// \param force
  void ShiftUp(bool force) {
    if (num_in_1 > 1000 || force) {
      for (int i = 0; i < 45; i++)
...
#endif  // DSL_MATRIX_ACCUMULATORS_H_
