...
//    p.z / maxDepth);
    ...
    return vec3((((cam.z * xm) + cam.x) - (cols * 0.5)) / (cols * 0.5),
        (((cam.w * ym) + cam.y) - (rows * 0.5)) / (rows * 0.5),
        p.z / maxDepth);
}
...
void main()
{
    ...
    if(vPosHome.z > maxDepth || !ValidFov(vPosHome.xyz, max_half_fov) /*|| vPosition.w < confThreshold || time - vColor.w > timeDelta || vColor.w > maxTime*/)
    {
    ...
    {
        //        float surfel_size = 0.01; // default surfel radius for splat
        float surfel_size = vNormRad.w;

        gl_Position = vec4(projectPoint(vPosHome.xyz), 1.0);

        colTime = vColor;
        position = vec4(vPosHome.xyz, vPosition.w);
        normRad = vec4(normalize(mat3(t_inv) * vNormRad.xyz), surfel_size /*vNormRad.w*/);
